...
void find_max_dims(
    ...
    for (char **p = argv_first; p != argv_last; p++) {
        ...
        std::cout<<std::endl<<"NEVENTS MAX  = "<<eventsN_max<<std::endl;
    ...
    char *argv_first[], char *argv_last[]) 
{
    ...
    for (char **p = argv_first; p != argv_last; p++) {
        ...
        int i = 0; //General event counter
        /* int i = 1764000; //General event counter */
        int iblock = 0; //Event countin in h5 file, incremented after write logic
        while (events.Next()) {
            ...
            //EDIT MAY 19 2023: iblock should only be incremented when the fill conditions are met!
...
for ( int si=0; si<n_subsystems; si++ ) 
            {
                subsysNHits[si] = subsysE[si] -> GetSize() ; 
            }
            ...
            for ( int si=0; si<n_subsystems; si++ ) {
                for (size_t h_hit = 0; h_hit < subsysNHits[si]; h_hit++) {
                    ...
                    if ( (*(subsysE[si]))[h_hit] <= 0.0006/2 ) continue ; //MIPS and Empty Cells
...
subsys_fill[si] ++ ;
            ...
            for (size_t particle = 0; particle < mcNParticles; particle++) 
            {
                ...
                cluster_data[iblock*cluster_row_size + 1] = mcTheta; // FIXME: add as a separate Gen dataset.
...
/* mc_data[(iblock*mc_row_size + 1)*mcNParticles_max + mc_fill] = mcSimulatorStatus[particle]; */ 
            ...
            //Either loop over detectors and sum, as below, or write different values per detector to disk

i++; //increment event counter
            //DECIDE TO WRITE EVENT
            if ( (event_hits == 0) || (mc_fill == 0) ){
                std::cout<<std::endl<<"Event Skipped"<<std::endl;
                /* std::cout<<std::endl<<eventsN_max<<std::endl; */
                continue;
            }

//FIXME: Put outside fo se loop, chang to  Nsub*row size

bool print_cal = false;
            bool print_mc = false;

            iblock++;
            if (iblock == (block_size)) 
            {
...
size_t mcNParticles_max = 0;
    ...
    /* eventsN_max = 1794000; cell_NHits_max = 1861; mcNParticles_max = 30; */
    eventsN_max = 1794000; cell_NHits_max = 200; mcNParticles_max = 30;
    eventsN_max = 100000; cell_NHits_max = 200; mcNParticles_max = 30;
